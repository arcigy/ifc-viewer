...
export class IFCManager {
    ...
    private setupLoader() {
        ...
        // Using relative path for better compatibility across different environments
        ...
        try {
            ...
            console.warn('Could not setup ThreeMeshBVH. Performance might be reduced.', e);
        }
    }
                    ...
                    (event: ProgressEvent) => {
                        if (onProgress && event.lengthComputable) {
                            ...
                            onProgress({
                                ...
                                status: percentage < 100 ? 'Downloading...' : 'Processing geometry...'
                            });
                        }
                    },
                    // onError
    ...
    public async getElementProperties(mesh: THREE.Mesh): Promise<any> {
        try {
            ...
            const propertySets = await this.loader.ifcManager.getPropertySets(modelID, expressID, true);
            ...
            try {
                ...
                if (properties.HasAssociations) {
                    for (const assoc of properties.HasAssociations) {
                        const assocProps = await this.loader.ifcManager.getItemProperties(modelID, assoc.value);
            ...
            if (properties.type) {
                try {
                    typeProperties = await this.loader.ifcManager.getItemProperties(modelID, properties.type);
                    // Also get Psets for the type
                    try {
                        const typePsets = await this.loader.ifcManager.getPropertySets(modelID, properties.type, true);
            ...
            if ((!propertySets || propertySets.length === 0) && properties.HasAssociations) {
                console.log('Manually searching for property sets in associations...');
                for (const assoc of properties.HasAssociations) {
                    try {
                        const assocProps = await this.loader.ifcManager.getItemProperties(modelID, assoc.value);
                        // IFCPROPERTYSET or IFCELEMENTQUANTITY
                        if (assocProps.type === 'IFCPROPERTYSET' || assocProps.type === 'IFCELEMENTQUANTITY') {
                            const fullSet = await this.loader.ifcManager.getItemProperties(modelID, assocProps.expressID);
    ...
    public getElementType(properties: any): string {
        ...
        const type = properties.type?.value || properties.constructor?.name || 'IFC Element';
